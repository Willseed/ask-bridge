...
`ask-bridge` 是一個基於 Rust 語言編寫的命令列工具，旨在透過 **Google Chrome** 的遠端除錯協定（Remote Debugging Protocol）來自動化操控網頁版的 **ChatGPT**、**Gemini**、**Claude** 與 **Grok**，並透過 MCP (Model Context Protocol) 與外部進行對接與問答。
